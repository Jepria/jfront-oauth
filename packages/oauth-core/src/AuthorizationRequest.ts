...
export class AuthorizationRequest {
  ...
  private authorizePKCE = (nonce: string): Promise<string> => {
    ...
    const codeVerifier = Crypto.getRandomString(43)
    this.meta.codeVerifier = codeVerifier
    this.storage.setItem(nonce, JSON.stringify(this.meta))
    return new Promise<string>((resolve, reject) => {
      try {
        Crypto.sha256(codeVerifier).then((result) => {
          //build request url
          resolve(
            `${this.authorizeUrl}?response_type=${
              this.responseType
            }&client_id=${this.clientId}&redirect_uri=${encodeURIComponent(
              this.redirectUri,
            )}&code_challenge=${result}&state=${nonce}`,
          )
        })
      } catch (error) {
        reject(error)
  ...
  private authorizeImplicit(nonce: string): Promise<string> {
    ...
    return new Promise<string>((resolve, reject) => {
      //build request url
      resolve(
        `${this.authorizeUrl}?response_type=${this.responseType}&client_id=${
          this.clientId
        }&redirect_uri=${encodeURIComponent(this.redirectUri)}&state=${nonce}`,
      )
    })
  }
