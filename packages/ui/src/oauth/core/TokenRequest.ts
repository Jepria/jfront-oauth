...
export class TokenRequest {
  ...
  withAuthorizationCode(authorizationCode: string): Promise<TokenResponse> {
    ...
    return new Promise<TokenResponse>((resolve, reject) => {
      axios.post(
        this.tokenUrl,
        `grant_type=authorization_code&client_id=${encodeURIComponent(this.clientId)}&redirect_uri=${encodeURIComponent(this.redirectUri)}&code=${encodeURIComponent(authorizationCode)}&code_verifier=${encodeURIComponent(codeVerifier as string)}`,
        {
          headers: {
      ...
      ).then(response => resolve(response.data)).catch(error => reject(error.response?.data));
    });
  }
  ...
  withRefreshToken(refreshToken: string): Promise<TokenResponse> {
    return new Promise<TokenResponse>((resolve, reject) => {
      axios.post(
        this.tokenUrl,
        `grant_type=refresh_token&client_id=${encodeURIComponent(this.clientId)}&refresh_token=${encodeURIComponent(refreshToken)}`,
        {
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/x-www-form-urlencoded'
  ...
  withUserCredentials(username: string, password: string): Promise<TokenResponse> {
    return new Promise<TokenResponse>((resolve, reject) => {
      axios.post(
        this.tokenUrl,
        `grant_type=password&client_id=${encodeURIComponent(this.clientId)}&username=${encodeURIComponent(username)}&password=${encodeURIComponent(password)}`,
        {
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/x-www-form-urlencoded'
